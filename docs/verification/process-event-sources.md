...
| interrupted publish recovery | the same suite leaves a store entry owner-writable exactly as a publish stopped between its rename and its final tightening would, and proves the next bind of that exact digest revalidates the complete tree and manifest, completes the tightening, and passes installed-tree verification, while a mutated entry at that mode keeps the mode-unsafe refusal |
