...
Process identity and the install form were re-verified on 2026-09-15 with omp 18.1.15, which installs as a bun script; see Detection.
...
| Fact | Value |
|---|---|
| Binary | 18.1.11 was `omp`, a single Bun-compiled executable resolved from `PATH` by `../../../bin/fm-spawn.sh`; a missing binary refuses the spawn. 18.1.15 installs as a bun script (`~/.bun/bin/omp` -> `@oh-my-pi/pi-coding-agent/dist/cli.js`, shebang `#!/usr/bin/env bun`), so the live process is `bun .../bin/omp` with `comm=bun` (verified 2026-09-15 on bun 1.4.2). |
...
| Marker | None of omp's own (verified on the 18.1.11 binary: `PI_CODING_AGENT` absent, no `PI_CODING_AGENT_DIR` or `OMP_PROFILE` in the default profile). `FM_OMP_HARNESS=omp` is Firstmate's launch marker; ancestry matches the exact process name `omp`, or the anchored `omp` script-path word in a bun interpreter's args for the 18.1.15 install form. |
...
`../../../bin/fm-harness.sh` tests `FM_OMP_HARNESS=omp` before `CLAUDECODE`, like Cursor's markers, and its ancestry walk matches the anchored process name `omp` above the interpreter fallback, then the anchored `omp` script-path word inside a bun interpreter's args (omp 18.1.15).
...
`../../../bin/fm-session-lock-lib.sh` matches the same anchored name and bun script-path form for session-lock ownership, and `../../../bin/backends/tmux.sh` classifies the exact `omp` name `agent` and attributes the bun form from its foreground args for liveness.
